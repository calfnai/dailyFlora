...
普通启动时关闭窗口不会退出 DailyFlora，而是隐藏到 Windows 系统托盘。右键托盘图标可以：

- 重新打开窗口；
- 立即显示一次花束；
- 设置关闭窗口后闲置 5、10、15、30 或 60 分钟自动显示，也可以关闭自动显示；
- 退出程序。

默认值是关闭窗口后闲置 10 分钟自动显示。自动显示使用 Windows 系统空闲时间和“隐藏窗口后的时间”双重条件；用户按 `Esc` 后退出全屏并回到托盘，不会结束后台程序。设置保存在当前 Windows 用户的 DailyFlora 数据目录中。

普通桌面窗口的 `Esc` 由 Electron 主进程兜底处理，因此即使 Windows 原生全屏状态下网页没有收到键盘事件，也会可靠退出全屏。
