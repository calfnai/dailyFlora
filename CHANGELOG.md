...
- Vercel 预览构建现在忽略 `gh-pages` 静态产物分支，避免 GitHub Pages 发布触发无意义的 preview failure 邮件；`main` 仍保留为源码快照健康检查，正式 Vercel 主版本继续来自 `codex/dailyflora-integration`。 / Vercel preview builds now ignore the static `gh-pages` artifact branch to avoid noisy preview-failure emails from GitHub Pages publishes; `main` remains a source snapshot health check while the primary Vercel release continues from `codex/dailyflora-integration`.
