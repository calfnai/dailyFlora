...
- 新增每日发布自动化 runbook，并明确 GitHub Pages 是主公开静态发布面，Vercel production 是 Git 集成生产验证面；输出纪律适用于长文档、构建日志、GitHub/Vercel/Curl 等所有噪声输出，不写成 Vercel 依赖中心。 / Added the daily publish automation runbook and clarified that GitHub Pages is the primary public static release surface while Vercel production is a Git-integration production verification surface; output discipline applies to all noisy long documents, build logs, GitHub/Vercel/curl output, not just Vercel.
...
本次验收版本命名为 0.71；新增 P 键快捷更换镜头旋转路径预设。 / This acceptance build is named 0.71; press P to quickly change the camera route preset.
...
- Clock mode 重新加入快速镜头让位动画：花束向左移动，为半透明时钟界面让出空间；同时移除 `/member/` 中无意义的 Credits 区块及错误旧截图引用。 / Restored a quick camera-clearing animation for Clock mode so the bouquet shifts left before the translucent clock surface settles; also removed the meaningless Credits section and incorrect legacy screenshot reference from `/member/`.
...
- 建立正式全站多语言基础系统：新增统一 locale 检测与 `dailyflora.locale.v1` 持久化、七语言圆形选择器、英文回退、静态页 `data-i18n` 应用器、SEO `lang/canonical/hreflang/Open Graph` 更新，并接入首页、About、Member、Platforms、Objects 与法律页面；DEV LOG 不进入正式入口。 / Added the formal site-wide localization foundation: unified locale detection with `dailyflora.locale.v1` persistence, seven-language circular selector, English fallback, a static-page `data-i18n` applicator, SEO `lang/canonical/hreflang/Open Graph` updates, and coverage for Home, About, Member, Platforms, Objects, and legal pages; DEV LOG is excluded from the formal entry points.
- 新增正式法律页面：Terms of Use、Privacy Policy、Credits & Attributions、Copyright Notice；文案按当前本机 MVP 的真实数据边界说明账户、收藏、参考图读取和生成记录。 / Added formal legal pages for Terms of Use, Privacy Policy, Credits & Attributions, and Copyright Notice, with copy aligned to the current local-MVP behavior for accounts, favorites, reference-image reading, and generated records.
- 重建公开入口的信息架构：统一首页、关于、个人花园、周边与线下、多端入口的导航顺序；`/signup/` 降级为 `/member/#signup` 兼容入口，会员页承担注册与登录后工作台。 / Rebuilt the public information architecture: homepage, About, Member Garden, Objects/Offline, and Downloads now share one navigation order; `/signup/` is reduced to a compatibility entry for `/member/#signup`, while Member owns both signup and signed-in workspace states.
- 将旧 `docs/member-test.html` 的上传识别与生成记录能力整合进 `/member/`：保留本机 MVP 边界，新增 `DF-MEMBER-...` 生成记录代码，并在收藏记录中显示 `DF-DATE-...` 日期代码。 / Integrated the old `docs/member-test.html` upload-recognition and generated-record flow into `/member/`: it keeps the device-local MVP boundary, adds `DF-MEMBER-...` generated-record codes, and shows `DF-DATE-...` date codes for saved bouquets.
- 修复日期选择越界：URL、原生日期输入、自定义日历和随机日期都会限制在今天及以前；未来日期按钮禁用。 / Fixed future-date selection: URL params, the native date input, custom calendar, and random-date button are all clamped to today or earlier, with future calendar days disabled.
- 为 About 与会员页增加纯花束嵌入模式 `?preview=1&embed=flower`，隐藏 HUD、版本标记、语言切换和控制 UI；页面本身仍通过 `/version.json` 保留版本代码。 / Added the clean bouquet embed mode `?preview=1&embed=flower` for About and Member, hiding HUD, release mark, language switcher, and controls while each page still exposes the release code through `/version.json`.
- 发布每日中文名重复 bug 的构建修复：默认中文名已由日期 seed、主题、flowerPlan 加上月标记和日标记生成，并新增 2026-07-08/2026-07-09 回归测试，确保花花自身中文名不再重复。 / Released the daily Chinese-name collision fix: default names now combine the date seed, theme, flowerPlan, month mark, and day mark, with a 2026-07-08/2026-07-09 regression test to keep each bouquet's own Chinese name unique.
- 将手势控制入口移入 `VIEW` 菜单，不再作为右上角独立悬浮按钮；进入时钟模式后 `VIEW` 仍保持可点击，折叠态只降低透明度，展开后可以继续关闭时钟、改日期、切换精度或启停手势。 / Moved the hand-control entry into the `VIEW` menu instead of keeping it as an independent upper-right floating button; in clock mode, `VIEW` remains clickable, only dimmed while collapsed, so users can still close the clock, change dates, switch quality, or toggle hand control.
- 用户复验否决飞燕草使用槭属来源的 Palmate 叶片，已将 `delphinium` 恢复为 `unresolved + none`；风信子 `hyacinth` 降级为 `review-needed`，只保留单独验收“风信子花序 + Strap 基生叶”；当前定稿 Strap 关系为洋水仙和狐尾百合。 / Owner review rejected the maple-derived Palmate leaf for delphinium, so `delphinium` is back to `unresolved + none`; hyacinth is downgraded to `review-needed` for a dedicated "Hyacinth inflorescence + Strap basal leaf" review, leaving narcissus and foxtail lily as the current approved Strap mappings.
- 修正风信子单花的垂直朝向梯度：下部盛开花略向下，中段外展，上部半开花和花蕾逐渐上扬，不再整穗近乎同一水平朝向。 / Corrected the hyacinth floret inclination gradient so lower open flowers droop slightly, middle flowers face outward, and upper half-open flowers and buds rise progressively instead of sharing one near-horizontal orientation.
...
- 新增“花型与叶片系统验收”工具：分层盘点 16 个原有抽象词表、25 种偏写实具名花型、1 个已通过混合花型和 1 个候选；支持逐项通过/不通过/待定、意见、本机保存和 JSON/Markdown 导出，并展示叶片接入主版本的流程范例；经 2026-07-25 复验当前进度为 2/25，风信子为 review-needed。 / Added the Flower & Leaf System Review tool: it separates 16 original abstract vocabulary entries, 25 realistic named forms, one approved hybrid, and one candidate; supports per-entry pass/reject/pending decisions, comments, device-local persistence, JSON/Markdown export, and documents the production foliage integration flow; owner review on 2026-07-25 sets the current progress to 2/25 with hyacinth marked review-needed.
- 固化 2026-07-25 用户对全部 43 个花型审核项的截图验收：32 项通过、8 项不通过、3 项保持待定、10 项含文字意见；按钮选择与原文意见分别保存，意见不得擅自改写按钮状态。褶边副冠水仙候选明确不通过并继续回炉，星环脉冲花继续保持“写实骨架 × 非现实配色”混合花型通过。 / Persisted the owner's screenshot-backed review of all 43 flower-system entries: 32 passed, 8 rejected, 3 remain pending, and 10 include written comments. Button selections and verbatim comments are stored separately so comments cannot silently rewrite a selected state. The Frilled Narcissus candidate is explicitly rejected for continued revision, while Orbital Pulse remains approved as a realistic-skeleton × unreal-palette hybrid.
- 修复审美复盘 Dashboard 的验收断路：复用规则、日期回炉、16 类词表、已验收混合花型、未通过候选、花库与参考任务等核心标题现在都直接显示可点击的 LAB 路径；日期回炉区提供 2026-07-07、07-11、07-13、07-16 四个高精度入口，候选文案同步改为“不通过、继续回炉”。 / Fixed the acceptance dead ends in the aesthetic-review Dashboard: core headings for reusable rules, date corrections, the 16-form vocabulary, approved hybrids, rejected candidates, the flower library, and reference tasks now expose explicit clickable LAB routes. The date-correction section links directly to the four high-quality 2026-07-07, 07-11, 07-13, and 07-16 views, and the candidate copy now correctly states rejected/continue revision.
- 修复 Vercel 受保护预览中的 Dashboard 内链：当入口 URL 带 `_vercel_share` 时，Dashboard 会把该访问 token 继承到同域 LAB、日期花束、写实花库和科幻花束入口；四个日期花束按钮也直接显示完整 `/?date=...&render=high&density=high` 路径。 / Fixed Dashboard deep links in protected Vercel previews: when the entry URL includes `_vercel_share`, the Dashboard now preserves that access token on same-origin LAB, daily-bouquet, realistic-library, and sci-fi-bouquet links; the four date buttons also show the full `/?date=...&render=high&density=high` route.
- Sites 私有版本在静态包缺少大体积手势模型时，从已验证的 Vercel 整合主线读取同版本 MediaPipe WASM 与识别模型，避免重复打包二进制资源；Vercel 主版本仍完整自托管这些资源。 / The private Sites build now falls back to the verified Vercel integration release for large MediaPipe WASM and recognizer assets when they are omitted from the static package, avoiding duplicate binary packaging while the Vercel production build remains fully self-hosted.
- Vercel 预览构建现在忽略 `gh-pages` 静态产物分支，发布脚本会把最小 ignore 配置复制到 Pages 产物分支，避免 GitHub Pages 发布触发无意义的 preview failure 邮件；`main` 仍保留为源码快照健康检查，正式 Vercel 主版本继续来自 `codex/dailyflora-integration`。 / Vercel preview builds now ignore the static `gh-pages` artifact branch, and the deploy script copies the minimal ignore config into the Pages artifact branch to avoid noisy preview-failure emails from GitHub Pages publishes; `main` remains a source snapshot health check while the primary Vercel release continues from `codex/dailyflora-integration`.
...
- 补齐浏览器手势控制相关 `src/dailyFloraHandControl.ts` 与 `src/hand-control/*` 到 GitHub source deploy manifest，避免 Vercel 从 `main` 干净构建时缺少 DailyFlora 手势适配模块。 / Added `src/dailyFloraHandControl.ts` and `src/hand-control/*` to the GitHub source deploy manifest so Vercel clean builds from `main` include the DailyFlora hand-control adapter.
- 2026-07-23 曾暂时将 Strap 映射洋水仙、风信子与狐尾百合，并将 Palmate 映射飞燕草；其中飞燕草关系已在 2026-07-25 用户复验后撤销。 / On 2026-07-23, Strap was mapped to narcissus, hyacinth, and foxtail lily while Palmate was temporarily mapped to delphinium; the delphinium relation was withdrawn after owner review on 2026-07-25.
...
- 新增独立“花型 × 叶片搭配验收”页面；页面当前展示两组已确认 Strap 关系和一组风信子待复验关系，25 种偏写实花型总览不再被误当作花叶搭配交付页。 / Added a dedicated Flower × Leaf Pairing Review page; it now shows two confirmed Strap relationships plus one hyacinth review-needed item, so the 25-form realistic flower overview is no longer misrepresented as the pairing deliverable.
...
- 修复未镜像摄像头帧造成的物理左右手通道反转；监视器现在常驻显示完整 DailyFlora 手势表、左右手追踪状态与实际输出，并提供可关闭的左右手校正开关，兼容已经镜像的虚拟摄像头。 / Fixed physical left/right channels being reversed for unmirrored webcam frames; the monitor now keeps the full DailyFlora gesture map, physical-hand tracking state, and actual output visible, with an optional handedness-correction switch for already-mirrored virtual cameras.
- 将难以稳定摆出的中指、无名指和小指 pinch 开关改为更直观的 `👍 ✊ ☝ ✌` 图形手势，并用 landmarks 增加三指竖起识别；食指 pinch 只保留为 XY 连续控制，放宽至符合真实手指厚度的近触阈值，握拳作为立即停止所有连续输出的安全刹车。 / Replaced difficult middle/ring/pinky pinch switches with clearer symbolic poses and added a landmark-derived three-finger pose; index pinch remains only for continuous XY control with a near-touch threshold that accounts for fingertip thickness, while a fist immediately brakes every continuous output.
- 将图形手势定稿为左右手通用，并新增“拇指收起、其余四指伸直”的沉浸全屏手势；depth 与半合掌 rotate 也支持任一单手，双手同现仍保留 spread，depth 方向按实测反馈整体反转。 / Finalized every symbolic command for either hand, added a folded-thumb four-finger immersive gesture, enabled depth and semi-closed-palm rotation from either hand while retaining two-hand spread, and reversed depth direction based on live testing feedback.
- 在页面右上角加入手掌形手势总开关：开启时才动态加载独立监视器，关闭时销毁控制器并释放摄像头；仅显示按钮不会加载 MediaPipe、WASM 或模型，`?hand-control=1` 继续用于自动打开测试。 / Added a palm-shaped master hand-control switch beside the page menu: enabling it lazily loads the isolated monitor, disabling it disposes the controller and releases the camera, and merely displaying the button does not load MediaPipe, WASM, or the model; `?hand-control=1` still auto-opens the test.
