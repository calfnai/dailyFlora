...
- `0.14.11` retry：在不改变真实后端状态的前提下，补齐首页 `VIEW` 帮助入口、键盘快捷键、本机会员页和公开条款/隐私/署名/版权说明；首页 `INDEX` 主菜单保持“关于、收藏今日花束、生成我的专属花束、周边与线下、多端同步、scifi、我的花园”及语言切换，不把使用方法塞进主菜单。 / `0.14.11` retry: added the home `VIEW` help entry, keyboard shortcuts, the device-local member page, and public terms/privacy/credits/copyright pages without changing the real backend status. The home `INDEX` menu stays scoped to About, save today's bouquet, generate my bouquet, objects/offline, multi-device sync, scifi, My Garden, and the language switcher.
- 继续补齐正式帮助系统：VIEW 控件统一提供可翻译的 `title`、`aria-label`、焦点/悬停 tooltip 与移动端长按提示；全屏教程区分本次关闭和永久不再自动提示，HOW TO USE 可恢复教程状态；开发 Dashboard 入口只在 `?debug=1` 时动态出现，SciFi 正式页面移除开发 LAB 链接，ABOUT 与 Credits 统一以 CALFN LAU 为创作者和版权主体。 / Continued the public help system: VIEW controls now expose translatable `title`, `aria-label`, hover/focus tooltips and a mobile long-press hint; fullscreen help separates a one-entry dismissal from permanent opt-out, and HOW TO USE can reset tutorial state. The review Dashboard is created only for `?debug=1`, the public SciFi page no longer links to a development LAB, and ABOUT/Credits consistently identify CALFN LAU as creator and rights holder.
- 修正首页普通状态的 UI 自动隐藏边界与构图：闲置后只把展开的观看工具收回到 `VIEW`；桌面端 `VIEW` 保留在右下，移动端移到右上 `INDEX` 下方，避免遮住左下标题和右下括号。左上、右下装饰括号分别内收至 36px / 26px 安全区并提高可见度。 / Corrected the home UI auto-hide boundary and composition: inactivity now only collapses the expanded tools back to `VIEW`; `VIEW` stays at the lower right on desktop and moves below `INDEX` on mobile so it cannot cover the lower-left title or lower-right bracket. The upper-left and lower-right brackets now sit within 36px / 26px safe areas with stronger visibility.
- 按指定层级重排首页五行标题：`DAILY COMPOSITION`；中文大字 `7月27日·晚晴雨林廿七橙梦幻紫空气`；英文小字 `JUL 27,2026 ·EVENING CLEAR RAINFOREST TWENTY-SEVENTH-ORANGE FAIRY VIOLET AIR·`；英文花材计划；中文花材计划。桌面展开观看设置时工具移到右上独立区域，不再压碎标题。 / Reordered the five-line home title to the requested hierarchy: `DAILY COMPOSITION`; the large Chinese date/title; the small English date/title; the English flower plan; and the Chinese flower plan. Expanded desktop tools move to a separate upper-right area instead of breaking the title layout.
- 将 `EN / 中 / ES / FR / PT / IT / 日` 从菜单标签切换扩展为整页 GUI 语言切换，覆盖观看工具、动态暂停/继续状态、手势状态、日历、时钟设置、日期、引语、tooltip 和 aria 文案；中英双行花束名称与花材计划作为作品标识保持指定顺序，不做未经审校的花名伪翻译。 / Expanded `EN / 中 / ES / FR / PT / IT / 日` from menu-only labels into a full-page GUI locale switch covering viewing tools, dynamic pause/resume state, hand-control state, calendar, clock settings, date, quote, tooltips, and aria copy. The bilingual bouquet title and botanical plan remain in the specified canonical order rather than receiving unreviewed pseudo-translations.
- 重做手动时钟关闭控件：桌面端放在右侧中部边缘，移动端放在右下安全区，使用低对比、无边框、无底色的细小关闭符号并避开时间正文；自动待机时钟不显示该控件。 / Restyled the manual-clock close control as a low-contrast borderless mark at the right-side edge on desktop and in the lower-right safe area on mobile, clear of the clock content; it remains absent from automatic idle-clock mode.
- 以 `0.14.11` 分支推进 UI 刷新：首页 `INDEX` menu 将“收藏今日花束”和新增“生成我的专属花束”统一指向 `/signup/`，并在菜单下方恢复 `EN / 中 / ES / FR / PT / IT / 日` 的界面标签切换。花束名称和花材内容不做未经审校的多语言伪翻译。 / Advanced the UI refresh as the `0.14.11` branch: the home `INDEX` menu now sends both "save today's bouquet" and the new "generate my bouquet" entry to `/signup/`, and restores the `EN / 中 / ES / FR / PT / IT / 日` interface-label switcher below the menu. Bouquet names and botanical content are not pseudo-translated.
- 重写 `/about/`、`/signup/`、`/member/`、`/docs/member-test.html`、`/bouquet-shop/` 和 `/downloads/` 的导航与可见文案：About 移除协作开发宣传段落，周边与线下恢复完整周边/线下计划，多端页面命名为“多端同步”，会员页首屏优先展示“我的收藏”。 / Reworked navigation and visible copy across `/about/`, `/signup/`, `/member/`, `/docs/member-test.html`, `/bouquet-shop/`, and `/downloads/`: About removes the collaboration-credit promo copy, Objects & Offline restores the fuller merchandise/offline roadmap, Downloads is renamed to "multi-device sync", and the member page leads with saved bouquets.
- 明确 Supabase 状态：当前 `/signup/` 和 `/member/` 仍是本机 localStorage MVP。旧 `codex/vercel-blob-supabase` 分支保留 Supabase/Blob 参考图记录后端雏形，但本版本不完成真实 Supabase Auth、跨端同步、支付订阅或云端 Credits。 / Clarified Supabase status: `/signup/` and `/member/` remain device-local localStorage MVP flows. The older `codex/vercel-blob-supabase` branch remains a reference for the Supabase/Blob image-record backend shape, but this release does not complete real Supabase Auth, cross-device sync, paid subscriptions, or cloud Credits.
