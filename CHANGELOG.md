...
- 用户复验否决飞燕草使用槭属来源的 Palmate 叶片，已将 `delphinium` 恢复为 `unresolved + none`；洋水仙、风信子和狐尾百合三组 Strap 关系保留并作为当前定稿映射。 / Owner review rejected the maple-derived Palmate leaf for delphinium, so `delphinium` is back to `unresolved + none`; the three Strap mappings for narcissus, hyacinth, and foxtail lily remain approved.
...
- 新增独立“花型 × 叶片搭配验收”页面；页面当前只展示三组已确认 Strap 关系，25 种偏写实花型总览不再被误当作花叶搭配交付页。 / Added a dedicated Flower × Leaf Pairing Review page; it now shows only the three confirmed Strap relationships, so the 25-form realistic flower overview is no longer misrepresented as the pairing deliverable.
