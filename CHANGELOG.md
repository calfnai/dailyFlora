...
- 修正首页普通状态的 UI 自动隐藏边界：闲置后只把展开的观看工具收回到 `VIEW`，左上构建标、右上 `INDEX`、左下五行中英文标题与右下 `VIEW` 持续显示；只有进入手动或自动时钟状态时，普通 UI 与界面角标才一起隐藏，关闭时钟后完整恢复。 / Corrected the home UI auto-hide boundary: normal inactivity now collapses expanded viewing tools back to `VIEW` while the upper-left build mark, upper-right `INDEX`, five-line lower-left title, and lower-right `VIEW` remain visible; ordinary UI and corner chrome hide together only in manual or automatic clock mode and return after the clock closes.
