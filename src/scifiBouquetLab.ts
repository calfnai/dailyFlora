...
function createBouquet(definition: BouquetDefinition) {
  ...
  for (let i = 0; i < flowerCount; i += 1) {
    ...
    const baseScale = flowerId === 'fractal-rift'
      ? 0.18
      : flowerId === 'orbital-color-control'
        ? 0.22
        : flowerId === 'helix-beacon'
          ? 0.19
          : 0.2;
  ...
  labels.innerHTML = bouquetDefinitions.map((definition, index) => `
    <article class="bouquet-cell" data-bouquet="${escapeHtml(definition.id)}">
      <div class="bouquet-copy">
        ...
        <p class="support">科幻主体：${definition.focus.map((item) => escapeHtml(sciFiById[item].cn)).join(' / ')}<br />少量支撑：${definition.support.map((item) => escapeHtml(item)).join(' / ')}</p>
      </div>
    </article>
  `).join('');
}
