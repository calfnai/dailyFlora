import"./modulepreload-polyfill-B5Qt9EMX.js";import{W as m}from"./three.module-DzvgNsjM.js";import{f as v}from"./floraPrimitives-C5qL2vqc.js";import{S as b,H as $,D as F,P as y,a as S,V as k}from"./three.core-BPWrdVeF.js";import"./random-CVcU7mnd.js";const h=new URLSearchParams(window.location.search).get("debug"),C=new URLSearchParams(window.location.search).has("debug")&&h!=="0"&&h!=="false";if(!C)throw document.body.innerHTML=`
        ...
        `}function D(r){document.querySelector("#role-grid").innerHTML=r.roles.map(t=>`
            ...
            `).join("")}function j(r){document.querySelector("#primitive-grid").innerHTML=r.primitiveGate.map(t=>{const e=t.ownerStatus?`<p><strong>用户判定：</strong>${a(t.ownerStatus)}</p>`:"",s=t.ownerFeedback?`<p><strong>反馈口径：</strong>${a(t.ownerFeedback)}</p>`:"",i=t.ownerAcceptance?`<p><strong>验收结论：</strong>${a(t.ownerAcceptance)}</p>`:"";return`
                <article class="panel">
                  <div>${f(t.status)}</div>
                  <h3>${a(t.humanName)}</h3>
              ...
              `}).join("")}function M(r){const t=r.reusableAestheticRules||[];document.querySelector("#rules-grid").innerHTML=t.map(e=>`
              <article class="panel">
                <h3>${a(e.title)}</h3>
                <p>${a(e.summary)}</p>
                <div class="subpanel">
            ...
            `).join("")}function q(r){const t=r.dailyBouquetCorrections||[];document.querySelector("#daily-correction-grid").innerHTML=t.map(e=>`
              <article class="panel">
                ...
                <a class="pill route" href="${a(e.url)}">打开日期花束 · ${a((e.url||"").replace("../","/"))}</a>
              </article>
            ...
            `).join("")}function x(){p.splice(0,p.length),Array.from(document.querySelectorAll("[data-shape-preview]")).forEach(t=>{const e=t.dataset.shapePreview,s=v[e];if(!s)return;const i=new b;i.add(new $("#fff4dc","#182014",1.9));const n=new F("#ffffff",2.25);n.position.set(2.2,3.2,4.6),i.add(n);const l=new y(38,1,.1,20);l.position.set(0,.35,4.1),l.lookAt(0,.02,0);const o=new m({canvas:t,antialias:!0,alpha:!0});o.outputColorSpace=S,o.setPixelRatio(Math.min(window.devicePixelRatio,1.4));const c=s({seed:`dashboard-target-shape:${e}`,position:new k(0,0,0),scale:e==="FoliageGrassBranch"?.92:e==="SpikeFlower"?.84:e==="FrilledNarcissusFlower"?.9:1.02,colorPalette:H[e]||["#ffffff","#f7d78a","#80ad65","#cc8b4f"],openness:["OrchidButterflyFlower","TrumpetThroatFlower","FrilledNarcissusFlower","DaturaTrumpetFlower","CallaCurledBract"].includes(e)?.95:.7,density:["UmbelMiniCluster","FullHydrangeaCloud","FruitPodCluster","FoliageGrassBranch"].includes(e)?1.08:.92,curvature:["SpikeFlower","FoliageGrassBranch","CallaCurledBract"].includes(e)?.86:.42,role:["SpikeFlower","FoliageGrassBranch"].includes(e)?"line":"secondary"});c.rotation.x=["DiskFlower","CosmosOpenFlower","LayeredDahliaFlower","RuffledRoseFlower","StarPinwheelFlower","TrumpetThroatFlower","FrilledNarcissusFlower"].includes(e)?-.7:e==="FoliageGrassBranch"?-.34:-.1,e==="FoliageGrassBranch"&&(c.rotation.z=-.38),i.add(c),p.push({renderer:o,scene:i,camera:l,model:c,canvas:t,primitive:e})})}function g(){const r=performance.now()*.001;p.forEach(({renderer:t,scene:e,camera:s,model:i,canvas:n,primitive:l},o)=>{const c=Math.max(1,n.clientWidth),u=Math.max(1,n.clientHeight);(n.width!==Math.floor(c*t.getPixelRatio())||n.height!==Math.floor(u*t.getPixelRatio()))&&(t.setSize(c,u,!1),s.aspect=c/u,s.updateProjectionMatrix()),i.rotation.y=r*.22+o*.18,(l==="HangingBellFruit"||l==="FoliageGrassBranch")&&(i.rotation.y=r*.16+o*.12),t.render(e,s)}),window.requestAnimationFrame(g)}function O(r){return`
        ...
        `}function E(r){const t=document.querySelector("#group-grid");t.innerHTML=r.reviewGroups.map(O).join("")}function U(){const r=[["all","全部"],["blocked","blocked"],["needs-work","needs-work"],["pass","pass"],["positive","正向"],["negative","反向"]];document.querySelector("#filters").innerHTML=r.map(([t,e],s)=>`
            ...
            `).join("")}function V(){const r=Array.from(document.querySelectorAll(".filter-button")),t=Array.from(document.querySelectorAll(".group-card")),e=document.querySelector("#empty");r.forEach(s=>{s.addEventListener("click",()=>{const i=s.dataset.filter;r.forEach(l=>l.setAttribute("aria-pressed",String(l===s)));let n=0;t.forEach(l=>{const o=i==="all"||l.dataset.status===i||l.dataset.kind===i;l.classList.toggle("is-hidden",!o),o&&(n+=1)}),e.hidden=n>0})})}L().then(r=>{R(r),D(r),M(r),q(r),G(r),A(r),N(r),x(),g(),j(r),E(r),U(),V()});
//# sourceMappingURL=aestheticReviewDashboard-BIO5bVWC.js.map
