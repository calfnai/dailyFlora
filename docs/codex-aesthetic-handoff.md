...
- 当前 npm 版本：`0.16.1`（`0.16` 已通过 owner 验收；本版本用于法律文本与署名验收）
- 当前产品层标记：`0.14`
- 当前主要同步分支：`main`
...
- 0.15 到 0.73 采用定向整合，不整体 merge 脏的 0.15 worktree；花、茎、叶继续共享 `PlantStemInstance`，未确认成员保持 `unresolved + none`。
...
2026-07-26 Dashboard 入口纠正：用户指出只有一大堆说明标题、没有对应 LAB 地址的复盘页没有验收价值。以后 Dashboard 的核心结论区块必须在标题旁直接显示可点击且带路径文字的操作入口，不能要求用户先滚动到资料库再猜对应页面。日期回炉记录必须直接给出对应日期的高精度地址；抽象词表、混合花型、候选花型、花库和参考任务必须分别直达 Primitive LAB、写实花型 LAB、花型验收、参考图库或固定花束 LAB。
