...
- `0.14.11` 首页 UI 纠错：普通花束状态闲置时只允许展开的观看工具自动收回到 `VIEW`，左上构建标、右上 `INDEX`、左下角五行中英文标题与装饰线、右下 `VIEW` 与装饰括号必须持续显示。只有进入手动或自动时钟状态时，普通 UI 与这些装饰元素才一起隐藏；时钟画面不再显示“钟 / 分后显示 / 自动显示”设置。小型关闭按钮只在手动打开时钟时显示，自动待机时钟不显示，关闭手动时钟后恢复首页 UI。
