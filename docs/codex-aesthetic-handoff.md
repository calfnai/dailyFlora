...
- GUI v1 只作用于普通每日花束页：黑色空间、左下无框花名信息线、右上 `INDEX`、右下 `VIEW`、左上与右下反向细线括号，以及 `EN / 中 / ES / FR / PT / IT / 日` 语言选择器。
- `INDEX` 继续承载既有站点路由，`VIEW` 继续承载日期、随机、全屏、缩放、密度、精度、镜头和时钟控制；极简外观不能删除功能。
- `clock-settings` 只允许出现在 `VIEW` 菜单的 `clock-control` 中；`.clock-overlay.is-manual.is-visible` 和自动时钟 overlay 只显示时间、日期与名言，不能再复制一组设置。
- Special Edition 完全排除在 GUI v1 改造范围之外，继续使用原 HUD、圆形入口、观看按钮、开场、文案、音频和构图。
...
2026-07-25 花型验收固化：用户用完整截图确认了 43 个花型审核项，仓库原始记录为 `data/flower-system-owner-review-2026-07-25.json`。结果是 32 项通过、8 项不通过、3 项待定、10 项含文字意见。必须分别保留“按钮选择”和“意见原文”，不得因为意见写了“通过”或“不通过”就擅自覆盖用户实际选择。当前三项待定是盘状花、郁金香/杯型和洋水仙；褶边副冠水仙候选明确不通过，不升级为第 17 类；星环脉冲花继续保持已通过的“写实骨架 × 非现实配色”混合分类。后续工作按原始记录中的 `action` 分流为直接入库、抽象改名后入库、重新发起写实调研、继续修改和渲染故障排查。
