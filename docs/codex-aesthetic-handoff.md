...
- 当前 npm 版本：`0.14.11`
- 当前产品层标记：`0.14`
- 当前 UI 刷新分支：`codex/dailyflora-0.14.11-ui-refresh`
...
- `0.14.11` 首页 UI 纠错：左上、右下装饰括号必须位于可见安全区，桌面内收 36px、移动端内收 26px。左下标题固定为五层：`DAILY COMPOSITION`；中文大字“日期·中文花名”；英文小字“英文日期 ·英文花名·”；英文花材计划；中文花材计划。`2026-07-27` 的验收文案分别是 `7月27日·晚晴雨林廿七橙梦幻紫空气` 与 `JUL 27,2026 ·EVENING CLEAR RAINFOREST TWENTY-SEVENTH-ORANGE FAIRY VIOLET AIR·`。桌面展开观看设置时面板移到右上独立区域，不得挤碎标题；移动端收起的 `VIEW` 位于 `INDEX` 下方，不得遮住标题或右下括号。
- `0.14.11` 时钟与语言纠错：小型关闭按钮只在手动打开时钟时显示，桌面位于右侧边缘中部，移动端位于右下安全区，均不得压住时间、日期或引语；自动待机时钟不得显示关闭按钮。语言切换必须覆盖整个首页 GUI 的静态和动态标签、tooltip、aria、日历、时钟日期及引语，不能只翻译 INDEX 菜单。花束标题和花材计划仍按指定的中英双行作品标识显示，不做未经审校的多语言花名翻译。
