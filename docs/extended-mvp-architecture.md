...
用户上传参考图并生成私人花花时，正式实现遵循
[`user-reference-generation-contract.md`](./user-reference-generation-contract.md)：每次生成保存一张主参考 WebP、一张小缩略 WebP 和一条追加式参数记录。图片进入私有对象存储，数据库不保存 Base64。

系统只把识别结果匹配到 DailyFlora 公共花材库，不为用户新建花材，也不在用户记录中保存或复制花材列表。用户记录只保存识别结果、颜色、构图、seed、Three.js 参数、所属用户和系统版本。
