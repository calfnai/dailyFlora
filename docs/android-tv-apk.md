...
Status: experimental branch `android-tv-apk-v0`.
...
```bash
npm run android:tv:open
```

## GitHub Actions build

The workflow can be run manually from GitHub Actions on the `android-tv-apk-v0` branch.
...
## Install on Android TV for testing

With ADB debugging enabled on the TV:
...
Some TV brands disable wireless ADB by default. In that case, use a USB connection or the TV brand's developer-mode instructions.
...
After the first APK artifact installs successfully, add a dedicated TV mode:
...
That mode should simplify UI, enlarge text, reduce interaction dependency, and expose only TV-safe controls.
