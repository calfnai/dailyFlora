...
function runCodex(referencePath, manifestPath, outputPath, task) {
  const prompt = [
    '你是 DailyFlora 0.74 Beta 的只读花束参数分析器。',
...
const server = createServer(async (request, response) => {
  ...
  if (request.method === 'GET' && request.url === '/') {
    ...
    response.end(JSON.stringify({ ok: true, service: 'DailyFlora 0.74 Beta worker', version: '0.74-beta.1', health: '/health', process: 'POST /process' }));
    return;
  }
  if (request.method === 'GET' && request.url === '/health') {
    response.writeHead(200, headers);
    response.end(JSON.stringify({ ok: true, version: '0.74-beta.1', queueRoot: root }));
    return;
  }
...
server.listen(port, '127.0.0.1', () => {
  process.stdout.write(`DailyFlora 0.74 Beta worker listening on http://127.0.0.1:${port}\nQueue: ${root}\n`);
});
