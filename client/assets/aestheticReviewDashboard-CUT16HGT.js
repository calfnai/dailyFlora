import{S as m,H as v,D as b,P as $,W as F,a as y,V as S,f as k}from"./floraPrimitives-C_NSaF-W.js";const h=new URLSearchParams(window.location.search).get("debug"),C=new URLSearchParams(window.location.search).has("debug")&&h!=="0"&&h!=="false";if(!C)throw document.body.innerHTML=`
        ...
        `}function E(r){const t=document.querySelector("#group-grid");t.innerHTML=r.reviewGroups.map(O).join("")}function U(){const r=[["all","全部"],["blocked","blocked"],["needs-work","needs-work"],["pass","pass"],["positive","正向"],["negative","反向"]];document.querySelector("#filters").innerHTML=r.map(([t,e],s)=>`
            ...
            `).join("")}function V(){const r=Array.from(document.querySelectorAll(".filter-button")),t=Array.from(document.querySelectorAll(".group-card")),e=document.querySelector("#empty");r.forEach(s=>{s.addEventListener("click",()=>{const i=s.dataset.filter;r.forEach(l=>l.setAttribute("aria-pressed",String(l===s)));let n=0;t.forEach(l=>{const o=i==="all"||l.dataset.status===i||l.dataset.kind===i;l.classList.toggle("is-hidden",!o),o&&(n+=1)}),e.hidden=n>0})})}L().then(r=>{R(r),D(r),j(r),q(r),G(r),A(r),N(r),x(),g(),M(r),E(r),U(),V()});
//# sourceMappingURL=aestheticReviewDashboard-CUT16HGT.js.map
